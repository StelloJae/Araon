...
Araon은 한국투자증권 KIS OpenAPI의 **실전투자 앱키와 앱시크릿**을 사용합니다.
Araon 자체에는 주문, 매매, 자동매매 기능이 없지만, KIS OpenAPI credential은
개인 키이므로 공개 저장소, 이슈, 채팅방에 올리지 않는 것이 안전합니다.
...
Araon은 public onboarding에서 모의투자 선택을 노출하지 않습니다. 모의투자
...
- 장중인데 실시간 tick이 적음  
  ...
  Araon은 REST polling fallback을 유지합니다.
