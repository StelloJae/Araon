...
- **Desktop packaging hardening**: macOS signing failure 원인은 `mac.identity=null`로 bundle signing을 완전히 skip하면서 Mach-O linker/ad-hoc signature metadata와 sealed resources 상태가 어긋난 것. `mac.identity="-"` ad-hoc signing으로 로컬 macOS app bundle은 `codesign --verify --deep --strict` 통과. `spctl`은 Developer ID/notarization이 없어서 여전히 rejected가 정상. Browser quarantine/Finder path와 Windows EXE 실행은 계속 pending.
