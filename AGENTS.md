...
- **P1 data-growth hardening**: `stock_signal_events` 90일 retention + max 200 read clamp / `stock_news_items` 24h stale + 7일 prune + sanitized fetch failure status / `price_candles` prune는 server start 후 daily maintenance로 호출 / `/runtime/data-health`가 signal/news/disclosure/phone growth와 candle prune 상태를 표시 / live KIS·WS·selected minute probe 0회
- **Alert/risk-control polish**: client alert rules support price/change/volume/volume-baseline-ratio crossings plus optional market-cap scope. StockDetailModal has selected-ticker quick presets for common alert rules. Recent alert delivery outcomes are kept in a localStorage ring buffer for UI diagnostics and the server keeps a sanitized in-memory Telegram delivery log. Phone alerts are optional through env-configured Telegram bridge (`ARAON_TELEGRAM_BOT_TOKEN`, `ARAON_TELEGRAM_CHAT_ID`); unconfigured bridge returns safe 409 and never logs raw token/chat values.
- **Operational hardening 2026-05-07**: background daily backfill marks zero-work results as `no_change` and cools those tickers for 6h to avoid per-minute empty loops / REST polling skips only connected WebSocket-covered tickers when realtime gates are enabled, preserving fallback otherwise / Settings data health shows no-work exclusions, disclosure cache staleness, and phone delivery status.
