...
- **Alert/risk-control polish**: client alert rules support price/change/volume/volume-baseline-ratio crossings plus optional market-cap scope. Phone alerts are optional through env-configured Telegram bridge (`ARAON_TELEGRAM_BOT_TOKEN`, `ARAON_TELEGRAM_CHAT_ID`); unconfigured bridge returns safe 409 and never logs raw token/chat values.
