...
- [ ] 다음 작업은 fresh-install default OFF와 로컬 persisted always-on ON의 구분을 유지하는 것이다. 현재 사용자 로컬은 `websocketEnabled=true`, `applyTicksToPriceStore=true`; fresh install 기본값은 false/false.
